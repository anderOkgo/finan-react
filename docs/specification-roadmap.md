...
Remaining candidates for a future pass: `nativeValidation.js`'s two uncovered lines (94.11% today — `onNativeInvalid`/`onNativeInput`, the DOM-event wrapper functions around the already-fully-tested `getNativeValidationMessage`), `Form.jsx` (414 lines, the main movement-entry form — highest remaining value, not started), and the rest of the component tree (all still at 0%).
